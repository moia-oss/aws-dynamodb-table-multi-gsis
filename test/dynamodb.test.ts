...
const TABLE_PARTITION_KEY: Attribute = { name: 'hashKey', type: dynamodb.AttributeType.STRING };
...
const GSI_PARTITION_KEY: Attribute = { name: 'gsiHashKey', type: dynamodb.AttributeType.STRING };
...
test('fails if encryption key is used with AWS managed CMK', () => {
  ...
  expect(() => new Table(stack, 'Table A', {
    ...
    encryptionKey,
  })).toThrow('encryptionKey cannot be specified unless encryption is set to TableEncryption.CUSTOMER_MANAGED (it was set to AWS_MANAGED)');
});
...
test('fails if encryption key is used with default encryption', () => {
  ...
  expect(() => new Table(stack, 'Table A', {
    ...
    encryptionKey,
  })).toThrow('encryptionKey cannot be specified unless encryption is set to TableEncryption.CUSTOMER_MANAGED (it was set to AWS_OWNED)');
});
...
describe('schema details', () => {
  ...
  test('if adding 2 GSIs,it create 2 custom GSIs builders with the right dependsOn', () => {
    // WHEN
    table.addGlobalSecondaryIndex({ indexName: 'GSI1', partitionKey: { name: 'gsi1', type: dynamodb.AttributeType.STRING } });
    table.addGlobalSecondaryIndex({ indexName: 'GSI2', partitionKey: { name: 'gsi2', type: dynamodb.AttributeType.STRING } });
...
test('error when enabling AutoScaling on the PAY_PER_REQUEST table', () => {
  // GIVEN
  const stack = new Stack();
  const table = new Table(stack, CONSTRUCT_NAME, { billingMode: dynamodb.BillingMode.PAY_PER_REQUEST, partitionKey: TABLE_PARTITION_KEY });
...
describe('metrics', () => {
  ...
  test('Can use metricSystemErrorsForOperations on a Dynamodb Table', () => {
    ...
    // THEN
    expect(stack.resolve(table.metricSystemErrorsForOperations({ operations: [dynamodb.Operation.GET_ITEM, dynamodb.Operation.PUT_ITEM] }))).toEqual({
  ...
  test('Using metricUserErrors with dimensions will fail', () => {
    ...
    const metric = table.metricUserErrors({ dimensionsMap: { TableName: table.tableName } });
...
describe('grants', () => {
  ...
  test('if table has an index grant gives access to the index', () => {
    ...
    const table = new Table(stack, 'my-table', { partitionKey: { name: 'ID', type: dynamodb.AttributeType.STRING } });
...
describe('secondary indexes', () => {
  ...
  test('attribute can be used as key attribute in one index, and non-key in another', () => {
    ...
    table.addGlobalSecondaryIndex({
      indexName: 'IndexA',
      partitionKey: { name: 'foo', type: dynamodb.AttributeType.STRING },
      projectionType: ProjectionType.INCLUDE,
    ...
    expect(() => table.addGlobalSecondaryIndex({
      indexName: 'IndexB',
      partitionKey: { name: 'baz', type: dynamodb.AttributeType.STRING },
      sortKey: { name: 'bar', type: dynamodb.AttributeType.STRING },
      projectionType: ProjectionType.INCLUDE,
